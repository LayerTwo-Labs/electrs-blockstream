...
/// Parsed data from a BIP300 drivechain deposit pegin witness.
#[derive(Copy, Clone, PartialEq, Eq, Debug, Hash)]
pub struct DrivechainPeginData<'tx> {
    /// Mainchain outpoint locked into the drivechain deposit.
    pub outpoint: bitcoin::OutPoint,
    /// Deposit value in satoshis.
    pub value: u64,
    /// Asset credited on the sidechain.
    pub asset: AssetId,
    /// Mainchain genesis hash committed by the sidechain.
    pub genesis_hash: bitcoin::BlockHash,
    /// Claim script carried by the import witness.
    pub claim_script: &'tx [u8],
    /// Mainchain deposit transaction id repeated by the drivechain marker.
    pub mainchain_txid: bitcoin::Txid,
}

impl<'tx> DrivechainPeginData<'tx> {
    const MARKER: &'static [u8] = b"drivechain-deposit-v1";

    /// Parse and validate the deterministic six-item drivechain deposit witness.
    pub fn from_pegin_witness(
        ...
        if pegin_witness[4] != Self::MARKER || pegin_witness[5].len() != 32 {
            return Err("invalid drivechain deposit marker");
        }

        let mainchain_txid = bitcoin::Txid::from_slice(&pegin_witness[5])
            .map_err(|_| "invalid mainchain txid")?;
        if prevout.txid != mainchain_txid {
            return Err("mainchain txid does not match pegin prevout");
        }

        let value: i64 = bitcoin::consensus::deserialize(&pegin_witness[0])
            .map_err(|_| "invalid value")?;
        if value <= 0 {
            return Err("value is not positive");
        }
...
impl TxIn {
    ...
    /// Extract a BIP300 drivechain deposit witness, if present and valid.
    pub fn drivechain_pegin_data(&self) -> Option<DrivechainPeginData<'_>> {
        self.pegin_prevout().and_then(|prevout| {
            DrivechainPeginData::from_pegin_witness(&self.witness.pegin_witness, prevout).ok()
        })
    }
